...
class InspectedCard extends PureComponent {
	render() {
		const { inspectedCard, bones } = this.props;
		...
		return (
			<TouchableWithoutFeedback onPress={this.props.onPress}>
				<View style={{borderWidth: 4, borderColor: "black", backgroundColor: "beige", width: 300, height: "180%", position: "absolute", top: "10%", left: "10%", }}>
					<Text style={{fontSize: 27, marginLeft: 10,}}>{inspectedCard.name}</Text><Text style={{position: "absolute", right: 10, fontSize: 30}}>{bones}</Text>
